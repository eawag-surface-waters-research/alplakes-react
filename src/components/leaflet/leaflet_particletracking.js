...
L.Control.ParticleTracking = L.Control.extend({
  options: {
    ...
    dt: 3, // number of computation timesteps between each data timestep
  ...
  _enableDrawing: function () {
    ...
    this._textbox.innerHTML = this.options.hover;
  ...
  _slideAlongBoundary: function (latlng, velocity, timestep, timeIndex) {
    var boundaryNormal = this._estimateBoundaryNormal(latlng);
    if (boundaryNormal === null) {
      return null;
    }
    var dotProduct =
      velocity.x * boundaryNormal.x + velocity.y * boundaryNormal.y;
    var tangentVelocity = {
      x: velocity.x - dotProduct * boundaryNormal.x,
      y: velocity.y - dotProduct * boundaryNormal.y,
    };

    var tangentSpeed = Math.sqrt(
    ...
    var subSteps = 5;
    var subTimestep = timestep / subSteps;
    var currentLatlng = latlng;
    var lastValidLatlng = latlng;
    var lastValidVelocity = tangentVelocity;

    for (let step = 0; step < subSteps; step++) {
      var testLatlng = this._moveLocation(
        currentLatlng,
        tangentVelocity,
        subTimestep,
      );
      var testVelocity = this._getVelocity(testLatlng, timeIndex);

      if (testVelocity !== null) {
        lastValidLatlng = testLatlng;
        lastValidVelocity = testVelocity;
        currentLatlng = testLatlng;
        var newNormal = this._estimateBoundaryNormal(testLatlng);
        if (newNormal !== null) {
          var newDot =
            testVelocity.x * newNormal.x + testVelocity.y * newNormal.y;
          tangentVelocity = {
            x: testVelocity.x - newDot * newNormal.x,
            y: testVelocity.y - newDot * newNormal.y,
          };
        } else {
          tangentVelocity = testVelocity;
        }
      } else {
        var validNearby = this._findNearestValidPoint(currentLatlng, timeIndex);
        if (validNearby !== null) {
          lastValidLatlng = validNearby.latlng;
          lastValidVelocity = validNearby.velocity;
          currentLatlng = validNearby.latlng;
        } else {
          break;
        }
      }
    ...
    if (lastValidLatlng !== latlng) {
      return {
        latlng: lastValidLatlng,
        velocity: lastValidVelocity,
      };
    }
    return null;
  },
  _estimateBoundaryNormal: function (latlng) {
    var sampleDist = this._xSize * 0.5;
    var samples = [];
    var numSamples = 8;

    for (let i = 0; i < numSamples; i++) {
      var angle = (i / numSamples) * Math.PI * 2;
      var testLat = latlng.lat + (sampleDist / 111320) * Math.sin(angle);
      var testLng =
        latlng.lng +
        (sampleDist / (111320 * Math.cos((latlng.lat * Math.PI) / 180))) *
          Math.cos(angle);
      var isValid = this._getIndexAtPoint(testLng, testLat) !== null;
      samples.push({
        angle: angle,
        valid: isValid,
        dx: Math.cos(angle),
    ...
    if (invalidCount === 0 || invalidCount === numSamples) {
      return null;
    }
  ...
  _findNearestValidPoint: function (latlng, timeIndex) {
    var searchDist = this._xSize * 0.3;
    var numAngles = 8;
    var numRadii = 3;

    for (let r = 1; r <= numRadii; r++) {
      var dist = (searchDist * r) / numRadii;
      for (let a = 0; a < numAngles; a++) {
        var angle = (a / numAngles) * Math.PI * 2;
        var testLat = latlng.lat + (dist / 111320) * Math.sin(angle);
        var testLng =
          latlng.lng +
          (dist / (111320 * Math.cos((latlng.lat * Math.PI) / 180))) *
            Math.cos(angle);
        var testLatlng = L.latLng(testLat, testLng);
        var velocity = this._getVelocity(testLatlng, timeIndex);
        if (velocity !== null) {
          return {
            latlng: testLatlng,
            velocity: velocity,
  ...
  _getRawVelocityAtCell: function (dataI, dataJ, timeIndex) {
    if (
      dataI < 0 ||
      dataI >= this._dataHeight ||
      dataJ < 0 ||
      dataJ >= this._dataWidth
    ) {
      return null;
    }

    if (timeIndex < 0 || timeIndex >= this._times.length) {
      return null;
    }

    var x = this._data[timeIndex][dataI][dataJ];
    var y = this._data[timeIndex][dataI][dataJ + this._dataWidth];
  ...
  _bilinearInterpolate: function (latlng, dataTimeIndex) {
    var gridI = this.options.nRows - (latlng.lat - this._yMin) / this._ySize;
    var gridJ = (latlng.lng - this._xMin) / this._xSize;
    var i0 = Math.floor(gridI);
    var i1 = i0 + 1;
    var j0 = Math.floor(gridJ);
    var j1 = j0 + 1;
    var di = gridI - i0;
    var dj = gridJ - j0;
    if (
      i0 < 0 ||
      i1 >= this.options.nRows + 1 ||
      j0 < 0 ||
      j1 >= this.options.nCols + 1
    ) {
      return null;
    }
    var t00 = this._transformationMatrix[i0]?.[j0];
    var t01 = this._transformationMatrix[i0]?.[j1];
    var t10 = this._transformationMatrix[i1]?.[j0];
    var t11 = this._transformationMatrix[i1]?.[j1];
    if (!t00 || !t01 || !t10 || !t11) {
      var nearestI = Math.round(gridI);
      var nearestJ = Math.round(gridJ);
      if (
        nearestI >= 0 &&
        nearestI < this.options.nRows + 1 &&
        nearestJ >= 0 &&
        nearestJ < this.options.nCols + 1
      ) {
        var tNearest = this._transformationMatrix[nearestI]?.[nearestJ];
        if (tNearest) {
          return this._getRawVelocityAtCell(
            tNearest[0],
            tNearest[1],
            dataTimeIndex,
          );
    ...
    var v00 = this._getRawVelocityAtCell(t00[0], t00[1], dataTimeIndex);
    var v01 = this._getRawVelocityAtCell(t01[0], t01[1], dataTimeIndex);
    var v10 = this._getRawVelocityAtCell(t10[0], t10[1], dataTimeIndex);
    var v11 = this._getRawVelocityAtCell(t11[0], t11[1], dataTimeIndex);
    var validCorners = [v00, v01, v10, v11].filter((v) => v !== null);
    ...
    if (validCorners.length < 4) {
      var avgX =
        validCorners.reduce((sum, v) => sum + v.x, 0) / validCorners.length;
      var avgY =
        validCorners.reduce((sum, v) => sum + v.y, 0) / validCorners.length;
      return { x: avgX, y: avgY };
    }

    var v0 = {
      x: this._lerp(v00.x, v01.x, dj),
      y: this._lerp(v00.y, v01.y, dj),
    };
    var v1 = {
      x: this._lerp(v10.x, v11.x, dj),
      y: this._lerp(v10.y, v11.y, dj),
    };

    return {
      x: this._lerp(v0.x, v1.x, di),
      y: this._lerp(v0.y, v1.y, di),
    };
  },
  _temporalInterpolate: function (v0, v1, t) {
    if (!v0 && !v1) return null;
    if (!v0) return v1;
    if (!v1) return v0;

    return {
      x: this._lerp(v0.x, v1.x, t),
      y: this._lerp(v0.y, v1.y, t),
    };
  },
  _getVelocity: function (latlng, time_index) {
    var numInterpolatedSteps = this._interpolated_times.length - 1;
    var numDataSteps = this._times.length - 1;
    var continuousDataIndex =
      (time_index / numInterpolatedSteps) * numDataSteps;
    var dataTimeIndex0 = Math.floor(continuousDataIndex);
    var dataTimeIndex1 = Math.min(dataTimeIndex0 + 1, numDataSteps);
    var temporalFraction = continuousDataIndex - dataTimeIndex0;
    var gridI =
      this.options.nRows - Math.round((latlng.lat - this._yMin) / this._ySize);
    var gridJ = Math.round((latlng.lng - this._xMin) / this._xSize);

    if (
      gridI < 0 ||
      gridI >= this.options.nRows ||
      gridJ < 0 ||
      gridJ >= this.options.nCols
    ) {
      return null;
    }
    ...
    if (this.options.useBilinearInterpolation) {
      if (doTemporalInterp) {
        ...
        return this._bilinearInterpolate(latlng, dataTimeIndex0);
      }
    }

    var t = this._transformationMatrix[gridI]?.[gridJ];
    if (t == null) {
      return null;
    }
